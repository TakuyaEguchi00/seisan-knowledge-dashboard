...
図表の集計は **受注日が 2023年5月1日〜2026年4月30日まで（暦で丸3年・36ヶ月）** に入る成約のみです（`ナレッジ/成約分析` の個別 `.md`、サマリー除外）。**年月別の表は成約が1件以上あった月だけ**表示し、2023-05〜2024-02のようにナレッジにファイルが無い期間は行を出しません。**暦月別トップ6／ワースト6／月×営業×都道府県**もすべてこの受注日範囲で暦月合算しています。再集計: **`scripts/dashboard-aggregate-3y.mjs`**（`node scripts/dashboard-aggregate-3y.mjs`）。
